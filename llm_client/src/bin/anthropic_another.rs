...
If there are no snippets which need to be probed then reply with an emply list of items for <code_to_probe_list>.
...
Remember to include the <code_to_probe_list> section, and keep the same XML format which we have told you about."#
        .to_owned();
    let request = LLMClientCompletionRequest::new(
