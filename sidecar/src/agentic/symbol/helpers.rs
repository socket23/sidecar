...
/// takes in output which contains search and replace blocks in a given range of code
/// and generates the final code after applying all the search and replace blocks
///
/// right now, there are no escape hatch and we just take the search block as it is
pub fn search_and_replace_generator(
    search_and_replace_output: &str,
    code_interested_in: &str,
    _code_range: &Range,
) -> String {
    let head = "<<<<<<< SEARCH";
    let divider = "=======";
    ...
    let re = Regex::new(&format!(
        r"(?s){}\n(.*?)\n{}\n(.*?)\n{}",
        head, divider, updated
    ))
    .expect("regex building should not fail");

    let mut original_updated = vec![];

    let mut code_lines = code_interested_in
        .lines()
        .into_iter()
        .map(|line| line.to_owned())
        .collect::<Vec<_>>();
    for capture in re.captures_iter(search_and_replace_output) {
        if capture.is_err() {
            continue;
        }
        let capture = capture.expect("is_error to work");
        let original = capture[1].to_string();
        let updated = capture[2].to_string();
        original_updated.push((original, updated));
    }

    // Now we want to check the lines which match the original section in the generated
    // code and see what we can update with the updated one
    for (original, updated) in original_updated.into_iter() {
        ...
        for i in 0..=code_lines.len() - original_lines_len {
            if code_lines[i..i + original_lines_len] == original_lines {
                ...
                result.push('\n');
                result.push_str(&code_lines[i + original_lines_len..].join("\n"));
                code_lines = result
                    .lines()
                    .into_iter()
                    .map(|line| line.to_owned())
...
mod tests {
    ...
    use super::search_and_replace_generator;

    #[test]
    fn test_regex_parsing() {
        let edited_code = r#"Certainly! I'll add a new method to `SymbolEventSubStepRequest` for creating a Document request. Here's the change:

/Users/skcd/scratch/sidecar/sidecar/src/agentic/symbol/tool_box.rs
```rust
<<<<<<< SEARCH
...
This change adds a new method called `create_document_request` to the `SymbolEventSubStepRequest` implementation. The new method has the same signature and functionality as the existing `document_request` method, creating a Document request with the provided parameters."#;
...
impl Something {
    ...
    "#;
        let replaced_code = search_and_replace_generator(
            &edited_code,
            &original_code,
            &Range::new(Position::new(0, 0, 0), Position::new(0, 0, 0)),
        );
        println!("{}", replaced_code);
        assert!(false);
    }

    #[test]
    fn test_regex_parsing_another() {
        let original_code = r#"#[derive(Debug, serde::Serialize)]
pub enum SymbolEventSubStep {
    ...
    Edit(SymbolEventEditRequest),
        ...
        let search_and_replace_blocks = r#"<thinking>
1. We need to add a new variant called Document to the SymbolEventSubStep enum.
2. We should create a new struct called DocumentationRequest to hold any necessary data for the documentation event.
3. We'll add the new variant using the DocumentationRequest struct.
</thinking>

Here's the *SEARCH/REPLACE* block to make this change:

/Users/skcd/test_repo/sidecar/sidecar/src/agentic/symbol/ui_event.rs
```rust
<<<<<<< SEARCH
...
pub struct DocumentationRequest {
    // Add fields as needed for documentation request
}
...
pub enum SymbolEventSubStep {
    ...
    GoToDefinition(SymbolEventGoToDefinitionRequest),
    Edit(SymbolEventEditRequest),
    Document(DocumentationRequest),
}
>>>>>>> REPLACE
```

This change adds the new Document variant to the SymbolEventSubStep enum, using a new DocumentationRequest struct. The DocumentationRequest struct is currently empty, but you can add fields to it as needed for the documentation functionality.
