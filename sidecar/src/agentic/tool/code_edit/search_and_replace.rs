            ...
            match self.search_block_status.clone() {
                SearchBlockStatus::NoBlock => {
                    if answer_line_at_index == head {
                        ...
                        let mut answer_lines = self.answer_to_show.lines().into_iter().map(|line| line.to_string()).collect::<Vec<_>>();
                        ...
                        let answer_lines_second_last = answer_lines[answer_lines_len - 2].to_owned();
                    ...
                    if answer_line_at_index == divider {
                        ...
                        match range {
                            Some(range) => {
                                ...
                                let accumualated_length = accumulated.lines().into_iter().collect::<Vec<_>>().len();
                                let mut answer_lines = self.answer_to_show.to_owned().lines().into_iter().map(|answer_line| answer_line.to_owned()).collect::<Vec<_>>();
                                let answer_lines_len = answer_lines.len();
                                /// TODO(skcd): Fix this
                            ...
                            None => {
                                ...
                                let answer_lines_len = answer_lines.len();
                                answer_lines.truncate(answer_lines_len - (accumualated_length + 2));
                                answer_lines.push("Failed to find relevant code snippet...".to_owned());
                                self.answer_to_show = answer_lines.join("\n");
                            }
                        };
                    } else {
                ...
                SearchBlockStatus::BlockFound((_, block_range)) => {
                        ...
                        .any(|updated_trace| *updated_trace == answer_line_at_index)
                    {
                        ...
                        let mut answer_lines = self.answer_to_show.lines().into_iter().map(|line| line.to_owned()).collect::<Vec<_>>();
                        answer_lines.pop();
                        answer_lines.push(format!("Edit completed: [L{}:{}]", block_range.start_line(), block_range.end_line()));
                        self.answer_to_show = answer_lines.join("\n");
                    } else {
