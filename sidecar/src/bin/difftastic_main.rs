/// Testing here if we can call difftastic with some information and get
/// back the generated diffs and they look okay
...
async fn main() {
   ...
   export const activateExtensions = async (context: ExtensionContext, languageTypes: Set<string>) => {
    ...
    /**
     * This is a rust comment about something important here
     */
    const isRustType = (fileExtension: string): boolean => {
        if (fileExtension === '.rs') {
            return true;
