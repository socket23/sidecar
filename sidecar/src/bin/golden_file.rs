    ...
    loop {
        tokio::select! {
            ...
            result = &mut initial_request_task => {
                match result {
                    Ok(symbols) => {
                        println!("initial_request_task completed successfully");
...
mod tests {
    ...
    struct ExpectedGoldenFile {
        id: String,
        golden_file_name: String,
    }

    #[tokio::test]
    async fn test_golden_file() {
        let expected_golden_files = vec![
            ExpectedGoldenFile {
                id: "1".to_string(),
                golden_file_name: "expected_golden_file_1.sql".to_string(),
            },
            ExpectedGoldenFile {
                id: "2".to_string(),
                golden_file_name: "expected_golden_file_2.sql".to_string(),
            },
            // Add more expected golden files as needed
        ];
                ...
                result = &mut initial_request_task => {
                    match result {
                        Ok(symbols) => {
                            ...
                            // Check if each expected golden file is in the returned symbols
                            for expected in &expected_golden_files {
                                let found = symbols.iter().any(|symbol| {
                                    symbol.file_path().ends_with(&expected.golden_file_name)
                                });
                                assert!(
                                    found,
                                    "Expected golden file '{}' (ID: {}) not found in the returned symbols",
                                    expected.golden_file_name,
                                    expected.id
                                );
                            }
                        }
