...
async fn main() {
    ...
    let primary_file = "/Users/zi/codestory/sidecar/sidecar/src/agentic/tool/plan/service.rs";
    let tool_box = _symbol_manager.tool_box();

    let associated_files = tool_box
        .find_associated_files(primary_file, message_properties)
        .await
        .unwrap();

    dbg!(&associated_files);

    let root_path = Path::new("/Users/zi/codestory/sidecar/sidecar/src");
    let tag_index = TagIndex::from_files(root_path, associated_files).await;
